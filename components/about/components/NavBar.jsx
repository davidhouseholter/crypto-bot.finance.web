...
export function NavBar() {
  ...
  useEffect(() => {
    function updateActiveIndex() {
      ...
      let  heroRect = document.getElementById('home-hero').getBoundingClientRect();
  ...
  return (
    <div ref={navBarRef} className="sticky top-0 z-50">
      ...
      <div style={{height: '8vh'}} className="hidden sm:flex sm:h-32 sm:justify-center sm:border-b sm:border-slate-200 sm:bg-white/95 sm:[@supports(backdrop-filter:blur(0))]:bg-white/80 sm:[@supports(backdrop-filter:blur(0))]:backdrop-blur">
        <ol
          role="list"
          className="mb-[-2px] grid text-center auto-cols-[minmax(0,15rem)] grid-flow-col text-base font-medium text-slate-900 [counter-reset:section]"
        >
          {sections.map((section, sectionIndex) => (
            <li key={section.id} className={classNames('flex [counter-increment:section]',
            'sub-nav',
            sectionIndex === activeIndex && sectionIndex > 0
                    ? 'active-sub-nav'
                    : ''
            )}>
              <Link
              ...
              >
                <p style={{width: '100%'}}>{section.title}</p>
              </Link>
            </li>
          ))}
        </ol>

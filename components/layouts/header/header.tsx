...
export default function Header() {
  ...
  const userIdentity = useSelector((state:any) => state.userIdentityMode.value)
  if(userIdentity) {
    
navigation = [
  { name: 'Home', href: '/', slug: '', current: false },
  { name: 'Trade Bots', href: '/trade-bots', slug: 'trade-bots', current: false },
  ...
  let nav = navigation.map(i => {i.current = false; return i;}).find(f => f.href == router.pathname ||  f.slug == router.pathname);
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navigation.map((item) => (
                <Disclosure.Button
                  ...
                  href={item.href}
                  className={classNames(
                    item.current ? 'bg-gray-900 text-white' : 'text-gray-p00 hover:bg-gray-700 hover:text-white',
